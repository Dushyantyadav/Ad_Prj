...
function onClickPay() 
{
	...
	<div class="srch">
		...
		<style>	
				...
				onclick=\"onClickPay()\"
				class='btn btn-default'>View Individual Sponsors
				</button>
				<br></br>
				<button style='background-color: #6db6b9e6;' type='submit' name='submit2' 
				onclick=\"onClickPay()\"
				class='btn btn-default'>View Organizations Sponsors
				</button>
	...
	<h2>List of events with their Sponsors</h2>
	<?php
	

		if(isset($_POST['submit']))
		{
			$q=mysqli_query($db,"SELECT * from event where ename like '%$_POST[search]%' ");
		...
		{
			$res=mysqli_query($db,"SELECT * FROM `event` ORDER BY `event`.`ename` ASC;");
			...
			if(isset($_SESSION['login_user']))
			{
				?>
					<script type="text/javascript">
						window.location="individuals.php"
					</script>
				<?php
			}
		}
