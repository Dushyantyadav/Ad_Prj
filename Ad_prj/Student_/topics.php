...
function closeNav() {
  ...
  document.body.style.backgroundColor = "white";
	...
	<h2>Topics Of Books</h2>
	<?php

		if(isset($_POST['submit']))
		{
			$q=mysqli_query($db,"SELECT * from topics where t_name like '%$_POST[search]%' ");

			if(mysqli_num_rows($q)==0)
			{
				echo "Sorry! No Topic found. Try searching again.";
			}
			else
			{
		echo "<table class='table table-bordered table-hover' >";
			echo "<tr style='background-color: #6db6b9e6;'>";
				//Table header
				echo "<th>"; echo "Topic ID";	echo "</th>";
				echo "<th>"; echo "Topic Name";  echo "</th>";
			echo "</tr>";	
		...
		{
			$res=mysqli_query($db,"SELECT * FROM `topics` ORDER BY `topics`.`t_id` ASC;");
