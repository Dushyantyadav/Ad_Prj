  ...
  document.body.style.backgroundColor = "white";
	...
	<div class="srch">
		<form class="navbar-form" method="post" name="form1">
				...
				<input class="form-control" type="text" name="search" placeholder="Search Rooms" required="">
	...
	<div class="srch">
		<form class="navbar-form" method="post" name="form1">
				...
				<input class="form-control" type="text" name="r_no" placeholder="Enter Room Number" required="">
	...
	<?php
		...
		{
			$q=mysqli_query($db,"SELECT * from rooms where r_no like '%$_POST[search]%' ");

			if(mysqli_num_rows($q)==0)
			{
				echo "Sorry! No Rooms found. Try searching again.";
			}
			else
			{
		echo "<table class='table table-bordered table-hover' >";
			echo "<tr style='background-color: #6db6b9e6;'>";
				...
				echo "<th>"; echo "Room Capacity";  echo "</th>";
			echo "</tr>";	

			while($row=mysqli_fetch_assoc($q))
			{
				echo "<tr>";
				echo "<td>"; echo $row['r_no']; echo "</td>";
				echo "<td>"; echo $row['cap']; echo "</td>";
		...
		{
			$res=mysqli_query($db,"SELECT * FROM `rooms` ORDER BY `rooms`.`r_no` ASC;");
			...
			if(isset($_SESSION['login_user']))
			{
				...
				?>
					<script type="text/javascript">
						window.location="request_room.php"
					</script>
				<?php
			}
			else
